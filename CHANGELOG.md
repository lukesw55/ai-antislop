...
- Rule fields `unless_preceded_by` and `unless_followed_by`. Unknown registry keys are rejected when the registry loads.
...
- `anti-slop/evals/rule-corpus.json`: 51 deterministic cases with exact rule, line, and decision expectations, run by `tests/test_eval_corpus.py`, which also requires a positive and a negative case for every rule.
...
Unit suite: 83 tests on Python 3.11.15 locally; CI covers Python 3.9, 3.11, and 3.13 on Ubuntu and 3.11 on Windows.
